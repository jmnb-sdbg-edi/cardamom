...
_IS_FT_AVAILABLE=`echo "@config.sel.min.cscis@" | grep FaultTolerance`

if [ "$_IS_FT_AVAILABLE" != "" ]; then
   # FT is available: test CCM with FT
   java -Xbootclasspath/p:${BOOTCLASSPATH} cdmw.ccm.assembly.FTAssemblyLoaderTest  $IRFLAG_ORB
   java -Xbootclasspath/p:${BOOTCLASSPATH} cdmw.ccm.assembly.FTAssemblyTest  $IRFLAG_ORB
else
