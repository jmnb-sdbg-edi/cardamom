...
#include <ccm_ft/CCM_ServerWithEvents_impl.hpp>
#include <Foundation/ossupport/OS.hpp>
#include <ccm_ft/HostnameResult_impl.hpp>
...
//
// IDL:thalesgroup.com/CcmFtTest/ServerModule/CCM_ServerWithEvents:1.0
//
...
//
void 
CcmFtTest::CCM_ServerWithEvents_impl::push_commands (CcmFtTest::EventTypeModule::Command * event)
    throw (CORBA::SystemException)
{
    std::cout << "   ... CCM_ServerWithEvents_impl::push_commands called with command " << event->commandName() << std::endl;
    
    if (strcmp(event->commandName(), CcmFtTest::EventTypeModule::COMMAND_KILL) == 0) {
        std::cout << "   ... CCM_ServerWithEvents_impl:  SUICIDE in 500 ms !!!!" << std::endl;
        SucideThread* thread = new SucideThread(500);
        thread->start();
    } else {
        if (strcmp(event->commandName(), CcmFtTest::EventTypeModule::COMMAND_GET_HOSTNAME) == 0) {
            std::string hostname = Cdmw::OsSupport::OS::get_hostname();
            std::cout << "   ... CCM_ServerWithEvents_impl:  return result " << hostname << std::endl;
            
            CcmFtTest::EventTypeModule::HostnameResult_var result = new CcmFtTest::HostnameResult_impl();
    ...
    throw(CORBA::SystemException)
{
    ...
    m_session_context = ::CcmFtTest::ServerModule::CCM_ServerWithEvents_Context::_narrow(ctx); 
}
