...
<!--
   ...
   @param none
-->
<xsl:template name="session_context_java">
   <!--
   ...
   <xsl:if test="boolean($lifecycleFrameworkNodes/home-impl) and $lifecycleFrameworkNodes/home-impl/@category = 'SESSION'">
      <xsl:if test="$_index &lt;= count($lifecycleFrameworkNodes/home-impl)">
         ...
         <xsl:if test="boolean($managedComponentNode)">
            <xsl:variable name="contextImplClassname" select="concat('CCM_', $managedComponentNode/@name, '_SessionContextImpl')"/>
            ...
            <xsl:document method="text" href="{$filename}">
               ...
               <!--
                  Insert the content.
               -->
               <xsl:call-template name="session_context_java.content">
         ...
         -->
         <xsl:call-template name="session_context_java">
            <xsl:with-param name="_index" select="$_index + 1"/>
         </xsl:call-template>
      </xsl:if>
   </xsl:if>
</xsl:template> <!-- end of template session_context_java -->
...
-->
<xsl:template name="session_context_java.content">
   ...
   <xsl:variable name="ccmSessionContextClassName">
...
import cdmw.ccm.cif.SessionContextImpl;

public class <xsl:value-of select="$_contextImplClassname"/> extends SessionContextImpl
   implements <xsl:value-of select="$ccmSessionContextClassName"/> {

    /**
     * Component servant implementation
     */
    private Session<xsl:value-of select="$_componentName"/>Impl component;

    public <xsl:value-of select="$_contextImplClassname"/>(org.omg.Components.CCMContext ctx,
        Session<xsl:value-of select="$_componentName"/>Impl component) {
        super(ctx);
...
</xsl:template> <!-- end of template session_context_java.content.1 -->



<!--
-->
<xsl:template name="session_context_java.content.2">
...
</xsl:template> <!-- end of template session_context_java.content.2 -->
