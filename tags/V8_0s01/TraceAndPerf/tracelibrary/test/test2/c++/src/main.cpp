             ...
             std::vector<std::string>& collectorNames)
{
    ...
    try {
        try {
            ...
            CosNaming::NamingContext_var defaultRootCtx =
            ...
            try {
                ...
                std::cerr << "DefaultRootContext does not exist" << std::endl;
            }

            NamingInterface ni_root(defaultRootCtx.in());

            // get global collector naming context
            
            CosNaming::Name collector_nc_;
            collector_nc_.length(4);
            collector_nc_[0].id = CORBA::string_dup("CDMW");
            ...
            do {
                     ...
                     ++iBinding)
                {
                    ...
                    std::cout << "found trace collector : " << collectorName.c_str() << std::endl;
                 ...
                 const std::vector<std::string>& collectorNames)
{
    ...
    try {
        ...
        producerName_[0].id = CORBA::string_dup (hostname.c_str());
