...
namespace
{
    ...
    const string GLOBAL_COLLECTOR_NAME       = "global_collector";
    const string GLOBAL_TRACE_FILE_NAME      = "test_trace_filter";
...
int main(int argc, char* argv[])
{
    ...
    OS::ProcessId idGlobalTraceCollector, idTraceLibrary; 
    ...
    if (collectorName == "no" || collectorName == "yes") {
        collectorName = GLOBAL_COLLECTOR_NAME;
    }
    ...
    if (traceFileName == "no" || traceFileName == "yes") {
        traceFileName = GLOBAL_TRACE_FILE_NAME; 
    }
    ...
    //
    // Format of trace is Horizontal
    //
    std::string formatOption = TRACE_FORMAT_OPTH.c_str();
...
#if (CDMW_ORB_VDR == tao)
        namingCorbaloc = " -ORBInitRef NameService=corbaloc:iiop:localhost:" +
    ...
    oss << " --collector-name=" << collectorName
        << " --trace-file-name=" << traceFileName
        << " --CdmwLocalisationService=" << collectorServPort
        << " --CdmwProcessPort=" << collectorCdmwPort
        << repositoryCorbaloc.c_str()
        << " " << TRACE_FORMAT_OPTION << "=" << formatOption;

    std::string traceCollectorArgs(oss.str());
    ...
    std::cout << gblTraceCltor << traceCollectorArgs << std::endl;

    idGlobalTraceCollector =
        OS::create_process(gblTraceCltor, traceCollectorArgs);
    ...
    std::string traceLibrary = "cdmw_test_trace_filter";
    ...
    try {
        if (status == SUCCESS) {
            ...
            std::string collectorAgentURL =
                "corbaloc::localhost:" + collectorServPort + "/GlobalCollectorAgent";
    ...
    status = auto_check(traceFileName, true);
