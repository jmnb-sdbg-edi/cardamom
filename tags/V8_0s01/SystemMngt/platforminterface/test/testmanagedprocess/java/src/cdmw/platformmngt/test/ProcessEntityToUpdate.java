 ...
 */
public class ProcessEntityToUpdate extends EntityToUpdate {

    /**
     * Constructor.
     */ 
    public ProcessEntityToUpdate(String entityName,
                                 String hostName,
        ...
        if (entityName.length() != 0 && !errorSet) {
            ...
            try {
                ...
                if (updatingCounter == ENTITY_UPDATE_MAX_COUNTER) {
                    ...
                    PlatformInterface.setProcessEntityStatus(applicationName,
                                                             processName,
                                                             hostName,
                                                             entityName,
                                                             "functioning",
                    ...
                    String status;   
                    status = PlatformInterface.getProcessEntityStatus(applicationName,
                                                                      processName,
                                                                      hostName,
                                                                      entityName,
                                                                      info);
                    ...
                    StringHolder info = new StringHolder();
                    String status;   
                    status = PlatformInterface.getProcessEntityStatus(entityName,
                                                                   info);
                    
                    if (status.equals("disfunction"))
                        {
                            PlatformInterface.setProcessEntityStatus(entityName,
                                                                  "functioning",
                                                                  "the process entity status is : functioning");
                        } else {
                        PlatformInterface.setProcessEntityStatus(entityName,
            ...
            } catch (com.thalesgroup.CdmwPlatformMngt.ProcessNotFound anf) {
                errorSet = true;
