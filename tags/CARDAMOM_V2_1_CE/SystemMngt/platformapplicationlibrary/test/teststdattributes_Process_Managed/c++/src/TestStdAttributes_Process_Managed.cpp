...
#include "teststdattributes_Process_Managed/TestStdAttributes_Process_Managed.hpp"
...
CPPUNIT_TEST_SUITE_REGISTRATION (TestStdAttributes_Process_Managed);
...
namespace Cdmw
{
  namespace PlatformMngt
  {
    ...
    TestStdAttributes_Process_Managed::~TestStdAttributes_Process_Managed ()
    {
...
*/
    ...
    void TestStdAttributes_Process_Managed::do_tests ()
    {
// //  set_nbOfRequestedTestOK (13);
      ...
      char flag_get_restart_type = 0;
      char flag_get_monitoring_info = 0;
      char flag_set_monitoring_info = 0;
	...
	try
	{
	  TEST_INFO ("Test on Process::restart_type: ");
	  CdmwPlatformMngt::RestartType myrestart;
	  myrestart = proc->restart_type ();
	  myrestart = CdmwPlatformMngt::HOT_RESTART;
	}
	catch (const CORBA::NO_IMPLEMENT & ex)
	{
	  flag_get_restart_type = 1;
	  CPPUNIT_ASSERT (1);	/* test ok */
	}

	if (flag_get_restart_type == 0)
	  CPPUNIT_ASSERT (0);	/*test not ok */


	TEST_INFO ("Test on Process::get_monitoring_info: ");
	CdmwPlatformMngt::CompleteMonitoringInfo monitor_info;

	try
	{
	  monitor_info = proc->get_monitoring_info ();
	  monitor_info.pull_monitoring_interval = 0;

	  flag_get_monitoring_info = 1;
	  CPPUNIT_ASSERT (1);	/* test ok */
	}
	catch (CdmwPlatformMngt::IncompatibleType & ex)
	{
	  flag_get_monitoring_info = 1;
	  CPPUNIT_ASSERT (1);	/* test ok */
	}

	if (flag_get_monitoring_info == 0)
	  CPPUNIT_ASSERT (0);	/*test not ok */




	TEST_INFO ("Test on Process::set_monitoring_info: ");
	try
	{
	  proc->set_monitoring_info (monitor_info);
	  flag_set_monitoring_info = 1;
	  CPPUNIT_ASSERT (1);	/* test ok */
	}
	catch (CdmwPlatformMngt::IncompatibleType & ex)
	{
