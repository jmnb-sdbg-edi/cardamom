 ...
 * HostEntityToUpdate.
 */
public class HostEntityToUpdate extends EntityToUpdate {

    /**
     * Constructor.
     */ 
    public HostEntityToUpdate(String entityName,
                              String hostName,
        ...
        if (entityName.length() != 0 && !errorSet) {
            ...
            try {
                ...
                if (updatingCounter == ENTITY_UPDATE_MAX_COUNTER) {
                    ...
                    PlatformInterface.setHostEntityStatus(hostName,
                                                          entityName,
                                                          "functioning",
                    ...
                    String status;   
                    status = PlatformInterface.getHostEntityStatus(hostName,
                                                                   entityName,
                                                                   info);
                    ...
                    StringHolder info = new StringHolder();
                    String status;   
                    status = PlatformInterface.getHostEntityStatus(entityName,
                                                                   info);
                    
                    if (status.equals("disfunction"))
                        {
                            PlatformInterface.setHostEntityStatus(entityName,
                                                                  "functioning",
                                                                  "the host entity status is : functioning");
                        } else {
                        PlatformInterface.setHostEntityStatus(entityName,
            ...
            } catch (com.thalesgroup.CdmwPlatformMngt.HostNotFound anf) {
                errorSet = true;
