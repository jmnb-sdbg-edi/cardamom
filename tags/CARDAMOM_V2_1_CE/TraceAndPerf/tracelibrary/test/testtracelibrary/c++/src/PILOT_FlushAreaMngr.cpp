    ...
    throw()
{
	...
	// create messages
    std::string message1 = "This is the first test message.";
    std::string message2 = "This is the second test message.";
    std::string message3 = "This is the third test message.";
    ...
    header.m_componentName = ""; // ECR-0123
    ...
    headerStringSize += header.m_componentName.length() + 1; // ECR-0123
    ...
    CdmwTrace::TraceProducer_var traceProducer =
        FlushAreaMngr::init (rootPOA.in(),
                             ...
                             flushingTime,
                             2,                     // 2 FlushArea
                             40 + headerStringSize, // Each FlushArea has 40 octets
    ...
    // message flooding
    TEST_INFO ("Deactivating the flush area manager");
    flushAreaMngr.deactivate_flushing();

    TEST_INFO ("Adding the second message 50 times");
    for (int i = 0; i < 50; ++i) {
        ret = flushAreaMngr.add_message (header, message2);
        TEST_CHECK (ret == true);
    }
    ...
    TEST_INFO ("Reactivating the flush area manager");
    flushAreaMngr.activate_flushing();

    TEST_INFO ("Waiting for the AreaFlusher to do its work");
    OsSupport::OS::sleep(timescale * flushingTime * 5);

    TEST_INFO ("Checking the number of messages received by the Collector");
    const CdmwTrace::FormattedMessageSeq& formattedMessagesSeq2 =
        pSim_CollectorServant->get_FormattedMessageSeq();
	nbMessages = formattedMessagesSeq2.length();
    TEST_INFO ("Nb received messages : " << nbMessages);
    TEST_INFO ("Received messages : ");
	for (CORBA::ULong i = 0 ; i < nbMessages ; i++) {
	    TEST_INFO ("  " << formattedMessagesSeq2[i].message.in());
	}
    TEST_CHECK (nbMessages == 6);

    TEST_INFO ("Deactivating the flush area manager");
    flushAreaMngr.deactivate_flushing();

    TEST_INFO ("Adding the first message 88 times");
    for (int i = 0; i < 88; ++i) {
        ret = flushAreaMngr.add_message (header, message1);
        TEST_CHECK (ret == true);
    }
    ...
    TEST_INFO ("Checking the number of messages received by the Collector");
    const CdmwTrace::FormattedMessageSeq& formattedMessagesSeq3 =
        pSim_CollectorServant->get_FormattedMessageSeq();
	nbMessages = formattedMessagesSeq3.length();
    TEST_INFO ("Nb received messages : " << nbMessages);
    TEST_INFO ("Received messages : ");
	for (CORBA::ULong i = 0 ; i < nbMessages ; i++) {
	    TEST_INFO ("  " << formattedMessagesSeq3[i].message.in());
	}
    TEST_CHECK (nbMessages == 8);

    TEST_INFO ("Deactivating the flush area manager");
    flushAreaMngr.deactivate_flushing();

    TEST_INFO ("Adding the third message 100 times");
    for (int i = 0; i < 100; ++i) {
        do {
            ret = flushAreaMngr.add_message (header, message3);
            if (!ret) {
    ...
    TEST_INFO ("Checking the number of messages received by the Collector");
    const CdmwTrace::FormattedMessageSeq& formattedMessagesSeq4 =
        pSim_CollectorServant->get_FormattedMessageSeq();
	nbMessages = formattedMessagesSeq4.length();
    TEST_INFO ("Nb received messages : " << nbMessages);
    TEST_INFO ("Received messages : ");
	for (CORBA::ULong i = 0 ; i < nbMessages ; i++) {
	    TEST_INFO ("  " << formattedMessagesSeq4[i].message.in());
	}
    TEST_CHECK (nbMessages == 10);
