...
<!--
   ...
   @param none
-->
<xsl:template name="session_home_java">
   <!--
      ...
      <xsl:if test="$_index &lt;= count($lifecycleFrameworkNodes/home-impl)">
         ...
         <xsl:document method="text" href="{$filename}">
            ...
            <!--
               Insert the content.
            -->
            <xsl:call-template name="session_home_java.content">
         ...
         -->
         <xsl:call-template name="session_home_java">
            <xsl:with-param name="_index" select="$_index + 1"/>
         </xsl:call-template>
      </xsl:if>
   </xsl:if>
</xsl:template> <!-- end of template session_home_java -->
...
-->
<xsl:template name="session_home_java.content">
...
public class <xsl:value-of select="$homeImplClassname"/> extends CCMHomeImpl
   implements <xsl:value-of select="$javaScopedHomeOperationsName"/> {
       ...
       <xsl:call-template name="findAllJavaInterfaceReadWriteAttributes">
          ...
          <xsl:with-param name="_template" select="'session_home_java.content.2'"/>
         ...
         <xsl:call-template name="findAllJavaHomeFactoryOperations">
            ...
            <xsl:with-param name="_template" select="'session_home_java.content.1'"/>
         </xsl:call-template>
         ...
         public org.omg.Components.CCMObject create_component_with_config_values(
                org.omg.Components.ConfigValue[] config)
            throws org.omg.Components.CreateFailure 
         {
            // actual java home implementation doesn't accept any 
            // config values for component creation.
            // (FT is not implemented with java)
            if (config.length > 0) {
                System.err.println("Unexpected FT Configuration values for non FT component creation");
                throw new org.omg.Components.CreateFailure(
                    com.thalesgroup.CdmwDeployment.UNEXPECTED_FT_CONFIG_VALUES.value);
            }
         ...
         protected org.omg.PortableServer.Servant create_component_servant(
            ...
            throws org.omg.Components.CreateFailure {
               ...
               try {
                  <xsl:value-of select="concat('Session', $managedComponentNode/@name, 'Impl')"/> delegate =
                     new <xsl:value-of select="concat('Session', $managedComponentNode/@name, 'Impl')"/>(compOid, ctx, compExec, isCreatedByFactoryOperation);
         ...
         protected cdmw.ccm.cif.CCMObjectImpl getCCMObjectImpl(
             ...
             try {
                 ...
                 return (<xsl:value-of select="concat('Session', $managedComponentNode/@name, 'Impl')"/>) tie._delegate();
...
</xsl:template> <!-- end of template session_home_java.content -->



<!--
-->
<xsl:template name="session_home_java.content.1">
...
</xsl:template> <!-- end of template session_home_java.content.1 -->


<!--
-->
<xsl:template name="session_home_java.content.2">
   <xsl:param name="_node"/>
...
</xsl:template> <!-- end of template session_home_java.content.2 -->
