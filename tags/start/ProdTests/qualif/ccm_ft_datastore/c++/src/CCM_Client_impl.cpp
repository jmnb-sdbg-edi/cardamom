...
#include <ccm_ft_datastore/CCM_Client_impl.hpp>

#include <ccm_ft_datastore/Command_impl.hpp>
...
CORBA::Long
CcmFtTest::CCM_Client_impl::get_data()
    throw (CORBA::SystemException)
{
    return 1;
}    
...
CcmFtTest::CCM_Client_impl::do_tests()
{
    ...
    test_connections(m_expectedPrimaryHostname.in(), data_step);
    ...
    data_step++;
    test_connections(m_expectedBackup1Hostname.in(), data_step);
    ...
    data_step++;
    test_connections(m_expectedBackup2Hostname.in(), data_step);
...
void
CcmFtTest::CCM_Client_impl::test_connections(const char* expectedHostname, long expectedData)
{
    ...
    try {
        ...
        TEST_INFO("host received :" << hostname);
        ...
        TEST_INFO("Test connections to ServerWithFacet group (expected data: " 
                  << expectedData << ")");    
        
        long data = hostInfo->get_data();
        TEST_INFO("data received :" << data);
        if (expectedData == data) {
            TEST_SUCCEED();
        } else {
            TEST_INFO("Server's hostname is not the one expected: " << data);
            TEST_FAILED();            
        }
