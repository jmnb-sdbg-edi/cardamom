...
BASIC_DEMO_DIR=$CDMW_RUNTIME_ROOT/ccmcif/java/test/integration
...
# copy idl file
echo Copy idl...
cp $CDMW_RUNTIME_ROOT/ccmcif/java/test/idl/Basic.idl $PACK_TMP_DIR

# make links for libraries
ln -s  $BASIC_DEMO_DIR/clientcomponent/prod/$ORB_JAVA/cdmwccmciftest.jar $PACK_TMP_DIR/libClientBasicDemo.jar
ln -s $BASIC_DEMO_DIR/servercomponent/prod/$ORB_JAVA/cdmwccmciftest.jar $PACK_TMP_DIR/libServerBasicDemo.jar

echo Create assembly archive

OLD_DIR=`pwd`
cd $PACK_TMP_DIR
zip -r $OLD_DIR/JavaBasicDemo.aar *
cd $OLD_DIR
