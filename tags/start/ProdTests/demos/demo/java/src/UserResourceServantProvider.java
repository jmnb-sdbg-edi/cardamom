...
public class UserResourceServantProvider extends ResourceServantProvider {
    ...
    public void releaseServant(byte[] oid,POA poa,Servant servant) {
        LifeCycleObjectBase removable = null;
        
        // The only created servants are DeviceImpl by create_device() 
        // or ApplicationPOATie by create_application().
        
        if (servant instanceof DeviceImpl) {
            // nothing to do, DeviceImpl doesn't use LifeCycleObjectBase
        
        } else if (servant instanceof ApplicationPOATie) {
            try {
                // ApplicationPOATie doesn't inherit from LifeCycleObjectBase
                // but its delegate object do !        
                ApplicationPOATie tie = (ApplicationPOATie) servant;
                removable = (LifeCycleObjectBase) tie._delegate();
            ...
            // Shouldn't happen !
            // This factory only creates DeviceImpl or ApplicationPOATie servants
