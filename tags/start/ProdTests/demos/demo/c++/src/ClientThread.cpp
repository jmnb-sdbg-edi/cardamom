...
#include "TestEntityStrategy.hpp"
#include "TestEntityStrategyRestart.hpp"
#include "TestUnManagedEntityStrategy.hpp"
#include "TestUnManagedEntityStrategyRestart.hpp"
#include "TestSessionStrategy.hpp"
#include "TestServiceStrategy.hpp"
#include "TestEventService.hpp"

#include "ProcessAgent.stub.hpp"
...
//----------------------------------------------------------------------------
const CORBA::ULong ClientThread::nb_commands = 15;
...
//----------------------------------------------------------------------------
CORBA::Long ClientThread::test_Event(const std::string & arg, std::ostream & os)
    throw()
{
    ...
    CosNaming::NamingContext_var nc = 
        Cdmw::NamingAndRepository::RepositoryInterface::get_domain_context(Cdmw::Common::Locations::CDMW_SERVICES_NAME_DOMAIN);
	 TestEventService event(m_orb.in(), nc.in());
	 event.start();
	 return OP_SUCCESS;
}
//----------------------------------------------------------------------------
    ...
    throw()
{
    ...
    CORBA::Long event     = test_Event(arg, os);
    return entity && unmanaged && session && service && event;
}
//----------------------------------------------------------------------------
