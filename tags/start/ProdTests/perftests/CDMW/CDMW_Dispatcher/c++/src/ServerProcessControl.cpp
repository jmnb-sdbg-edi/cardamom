    ...
    throw(CORBA::SystemException)
{
    ...
    Dispatcher::Tester_var tester = m_Tester_impl->_this();
    ...
    // Get Dispatcher name domain from repository
    CdmwNamingAndRepository::NameDomain_var DispatcherDomain;
    try {
        DispatcherDomain = repository->resolve_name_domain ("perf_test");
    ...
    try {
        ...
        CdmwNamingAndRepository::NameDomain::RegistrationId_var regId 
            = DispatcherDomain->new_name (registration_name.c_str());
    
        // register the object
        DispatcherDomain->register_object (regId.in(), tester.in ());
