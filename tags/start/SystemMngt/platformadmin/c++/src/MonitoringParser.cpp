 ...
 * ActivityPoint class.
 */
ActivityPointParser::ActivityPointParser(
    const DOM_Node& activityPointParser )
: ConfigElementParser( activityPointParser )
{
}

std::string ActivityPointParser::name() const
{
    return get_attribute_value( "Name" );
}

unsigned long ActivityPointParser::monitoring_interval() const
{
    return atol( get_attribute_value( "Monitoring-interval" ).c_str() );
}

unsigned long ActivityPointParser::monitoring_timeout() const
{
    return atol( get_attribute_value( "Monitoring-timeout" ).c_str() );
}

/**
 * ActivityPointContainer class.
 */
ActivityPointContainerParser::ActivityPointContainerParser(
    const DOM_Node& activityPointContainerParser )
: ConfigElementParser( activityPointContainerParser )
{
}

unsigned int ActivityPointContainerParser::get_activity_point_count() const
{
    return get_child_count( "Activity-point" );
}

ActivityPointParser ActivityPointContainerParser::get_activity_point(
    unsigned int activityPointIndex ) const
{
    DOM_Node activityPointParser(
        get_child_element( "Activity-point", activityPointIndex ) );

    return ActivityPointParser( activityPointParser );
}
