...
//  
SessionContext_impl::SessionContext_impl(Components::CCMContext_ptr ctx)
throw (CORBA::SystemException)
: CCMContext_impl(ctx)
{
    m_session_context = Components::SessionContext::_narrow(ctx);

    if (CORBA::is_nil(m_session_context.in()))
    {
        // This is to avoid Assertion failure in debug mode as internal ref count
        // equals 1 (CORBA C++ mapping oblige!)
        _set_ref_count(0);

        throw CORBA::BAD_PARAM(OrbSupport::BAD_PARAMWrongContextType, 
                               CORBA::COMPLETED_NO);
    }
}
...
{
    return m_session_context->get_CCM_object();
}
