...
usage()
{
    ...
    echo "Description:"
    echo "      This program starts the Trace Test."
}
...
# start trace library test exe

TRACETEST_EXE=$CDMW_RUNTIME_ROOT/tracecollector/c++/test/prod/$ORB_CPP/$BINARY_TYPE/tracetest
SIMREPOSITORY_PATH=$CDMW_RUNTIME_ROOT/simrepository/c++/prod/$ORB_CPP/$BINARY_TYPE
TRACECOLLECTOR_PATH=$CDMW_RUNTIME_ROOT/tracecollector/c++/prod/$ORB_CPP/$BINARY_TYPE
TRACEAPPLI_PATH=$CDMW_RUNTIME_ROOT/tracelibrary/c++/test/test1/prod/$ORB_CPP/$BINARY_TYPE

###
rm -f test_trace.*

PATH=$SIMREPOSITORY_PATH:$TRACECOLLECTOR_PATH:$TRACEAPPLI_PATH:$PATH
export PATH
$TRACETEST_EXE --NameServicePort=$NAMPORT --RepositoryPort=$REPPORT --CollectorServicePort=$COLPORT1 --CollectorCdmwPort=$COLPORT2
