...
usage() {
    ...
    msg="$msg -s <import_csci_delivery script> [-c csci1,...,csciN] [-r cvsroot] [-n]"
    echo "$msg"
    echo "options:"
    ...
    echo "    -c <csci1,...,csciN>                      : In adding of the modified CSCIs processed by this script, just "
...
for csci in `echo $csci_list`; do
    ...
    ## Determine the branch name for CSCI import
    ## and tags for branch and trunc
    ...
    # Deleted dir are identified on lines begining by a single - 
    # del_dir=`diff -u $tmp_dir/list1 $tmp_dir/list2 | grep '^-[^-]' | sed -e 's:-::'`
