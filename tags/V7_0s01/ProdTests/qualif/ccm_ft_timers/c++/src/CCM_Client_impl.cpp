...
#include <ccm_ft_timers/CCM_Client_impl.hpp>
...
CcmFtTest::CCM_Client_impl::test_connections(const char* expectedHostname)
{
    ...
    try {
        TEST_INFO("Test : the Client receives the info by the Timer attivated on PRIMARY (expected primary host: " 
                  << expectedHostname << ")");    
    ...
    TEST_INFO("======= Check Periodic Execution ===== " );
    CORBA::String_var currtime = hostInfo->get_currentTime();
    TEST_INFO("================================" );
    TEST_SUCCEED();
