...
unset DONE_LIBS
echo "check the library already load by the daemon and the system"
pmap_daemon.sh
...
declare -a REPOSITORY_PID=(`ps -f -ww -u root |grep '[c]dmw_naming_and_repository ' |grep -o ^root[\ ]*[0-9]* |grep -o [0-9]*$`)
declare -a FTREPOSITORY_PID=(`ps -f -ww -u root |grep '[c]dmw_ft_naming_and_repository ' |grep -o ^root[\ ]*[0-9]* |grep -o [0-9]*$`)
declare -a FTREPOSITORYPROXY_PID=(`ps -f -ww -u root |grep '[c]dmw_naming_and_repository_proxy ' |grep -o ^root[\ ]*[0-9]* |grep -o [0-9]*$`)

echo $REPOSITORY_PID
echo $FTREPOSITORY_PID    
echo $FTREPOSITORYPROXY_PID

if [ ${#REPOSITORY_PID[*]} -ne 1 ]
then
    if [ ${#FTREPOSITORY_PID[*]} -ne 1 ]
    then
	echo "Exactly one repository or ftrepository must be running "
	exit -1
    fi
fi
...
if [ ${#REPOSITORY_PID[*]} != "" ]
then
    echo "The repository pid is REPOSITORY_PID."
    process_pid $REPOSITORY_PID
fi

if [ ${#FTREPOSITORY_PID[*]} != "" ]
then
    echo "The FaultTolerance repository pid is FTREPOSITORY_PID."
    process_pid $FTREPOSITORY_PID    
    echo "The FaultTolerance proxy repository pid is FTREPOSITORYPROXY_PID."
    process_pid $FTREPOSITORYPROXY_PID
...
echo
echo "memory used by the repository"
