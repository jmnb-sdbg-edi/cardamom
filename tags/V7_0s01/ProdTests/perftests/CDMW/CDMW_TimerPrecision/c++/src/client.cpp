...
int main(int argc, char* argv[])
{
    ...
    //deltaT (msec) of the waking ups
    unsigned long deltaT = 10;

    //delay for the waking ups
	...
	if ((fd = fopen("client.cfg", "r"))!=NULL) {
		fscanf(fd,"%u",&niter);  // number of waking ups requested to the Timer
		fscanf(fd,"%lu",&deltaT);// deltaT (msec) of the waking ups
		fscanf(fd,"%lu",&delay);// delay for the waking ups
		if (ferror(fd)) {
    ...
    std::cout << "deltaT (usec/10) = " << deltaT << std::endl;
    std::cout << "delay  (msec)    = " << delay  << std::endl;
	...
	Cdmw::clock::CosClock::init(orb.in(),argc,argv);
        ...
        // ===================================================
            Cdmw::CdmwInit::ProcessControl_var process_ctrl =
              new TimerPrecision::ClientProcessControl(orb.in(),
                                                 niter,
                                                 deltaT,
												 delay);
        // ===================================================
