...
namespace {
     ...
     const char* MY_PERF_FILE = "TestEventPerf.txt";
...
public:
           ...
           poa_(PortableServer::POA::_duplicate(poa))
    {
        ...
        {
            std::cout << "Exception raised during PushConsumer_impl init" << std::endl;
                  ...
                  virtual public PortableServer::RefCountServantBase
{
    CORBA::ORB_var orb_; // The ORB
    PortableServer::POA_var poa_; // My POA.
    TypedEcho_var consumer_;  // consumer on eventchannel2
...
public:
           ...
           poa_(PortableServer::POA::_duplicate(poa))
    {
        ...
        try
        {
            ...
            consumer_ = 
                evtchannel->get_typedPushConsumer<TypedEcho>(INTERFACE_REF,
                                                  CosEventComm::PushSupplier::_nil());

            //consumer_ = TypedEcho::_narrow(obj);
        }
        catch (...)
        {
            std::cout << "Exception raised during Echo_impl init" << std::endl;
            std::cerr << " **** TEST FAILED AT LINE " << __LINE__ 
                      << " IN FILE: " << __FILE__ << std::endl;
        }
...
int main(int argc, char* argv[])
{
    ...
    OS::ProcessId idEventAdmin = 0;
#if (CDMW_ORB_VDR == orbacus )
    OS::ProcessId idEventServer = 0;
    OS::ProcessId idIrServ = 0;
    OS::ProcessId idTypedEventServer = 0;
#endif
    ...
    try
    {
         ...
         if (unit_tests) 
        {
            ...
            std::string namingCorbaloc =
                " -ORBInitRef NameService=corbaloc::localhost:" + nameServicePort + "/NameService";
        ...
        PortableServer::POA_var rootPOA = PortableServer::POA::_narrow(obj.in());
         ...
         CdmwPerformanceAdmin::PerformanceAdmin_var perfoMngrRef 
              = Cdmw::Performance::PerformanceMngr::init(rootPOA.in(), MY_PERF_FILE);
...
#if (CDMW_ORB_VDR == orbacus )
        // Create consumer 
        Echo_impl* typedpcImpl = new Echo_impl(orb.in(), rootPOA.in(), ni.name_context());
