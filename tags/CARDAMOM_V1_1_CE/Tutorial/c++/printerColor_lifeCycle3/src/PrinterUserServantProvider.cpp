...
namespace 
{
    const char* PRINTER_REP_ID = "IDL:thalesgroup.com/PrinterModule/ColorPrinter:1.0";
    
    const char* BW_PRINTER = "Bw_Printer_";
    const char* COLOR_PRINTER = "Color_Printer_";
};
    ...
    throw ()
{
    ...
    ostr << BW_PRINTER << device << "_" << oid_counter << "_" << timeval.seconds;
...
PortableServer::ObjectId* PrinterUserServantProvider::create_color_printer (const char* device)
    throw ()
{
	//
    // create an unique oid
    ...
    ostr << COLOR_PRINTER << device << "_" << oid_counter << "_" << timeval.seconds;
    
	PortableServer::ObjectId_var oid = 
	         PortableServer::string_to_ObjectId(ostr.str().c_str());
	
   	std::cout << "<<PrinterUserServantProvider>> Color Printer '" 
   	          << ostr.str().c_str() <<"' OID created" << std::endl;
          ...
          CORBA::SystemException)
{
    ...
    ColorPrinter_impl* printer_impl = 
                   new ColorPrinter_impl(m_object_deactivator);
    
    
    CORBA::String_var oid_string =
              PortableServer::ObjectId_to_string (oid);
                  
    std::string type = oid_string.in();
    
    if (type.compare (0, strlen(COLOR_PRINTER), COLOR_PRINTER) == 0)
    {
        printer_impl->set_color();
    }
    else
    {
        printer_impl->set_blackAndWhite();
    }
...
{            
    ...
    std::cout << "<<PrinterUserServantProvider>> ColorPrinter servant reference removed." << std::endl;
}
