...
#include "lifecycle/UserAnonymousResourceServantProvider.hpp"
#include "lifecycle/Device_impl.hpp"
    ...
    throw(CF::Device::InvalidLocation)
{
    try {
        ...
        ECHO_DEF << "DEVICE <" << ostr.str() << "> created with LABEL='" << label << "'" << std::endl;
    ...
    throw(CF::CreateApplicationError)
{
    try {
        ...
        ECHO_DEF << "APPLICATION <" << oid_s.str() << "> created with NAME='" 
             << name << "'" << std::endl;
        return oid._retn();
    } catch (...) {
        throw CF::CreateApplicationError();
...
{    
    try  {
        ...
        ECHO_DEF << "Activating " << s.in() << " ... " << std::endl;
        return servant;
    ...
    throw(CORBA::SystemException)
{
    ...
    try  {
        CORBA::String_var s = PortableServer::ObjectId_to_string(oid);
        servant->_remove_ref();
        ECHO_DEF << "Deactivating " << s.in() << " ... " << std::endl;
