    ...
    throw (FT::InvalidState)
{
    ...
    m_nb_hello = s[0];
	 ...
	 // copy ulong
	 memcpy(&m_nb_hello, s.get_buffer(), sizeof(m_nb_hello));
	 
    CORBA::String_var str = CORBA::string_alloc(s.length() - sizeof(m_nb_hello) + 1);
    memcpy(str.inout(), s.get_buffer() + sizeof(m_nb_hello) , s.length());
    str[s.length() - sizeof(m_nb_hello) + 1] = '\0';

	 std::string string(str.in());

	 memcpy(str.inout(), s.get_buffer() + sizeof(m_nb_hello) + string.length() + 1, s.length());
	 str[s.length() - sizeof(m_nb_hello) - string.length() - 1] = '\0';
	 
	 std::string string10(str.in());
	 
    std::cout << "           . set new state to " << m_nb_hello
		        << ", " << string << ", " << string10 << std::endl;
}
