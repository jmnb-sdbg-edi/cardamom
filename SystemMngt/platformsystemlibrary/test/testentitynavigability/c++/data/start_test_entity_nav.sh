...
# @brief script to start the testentitynav
...
#insert the hostname in the file SystemDefinition.xml
sed -e 's/@host_name@/'$HOSTNAME'/' SystemDefinition.xml >ftemp
\mv -f ftemp SystemDefinition.xml 

echo "--------------------------------------------------"
echo "Starting scenario definition ..."
cdmw_platform_admin --system-corbaloc=corbaloc::localhost:21871/CdmwPlatformMngtSupervision --sys-define SystemDefinition.xml &

sleep 10
echo "Wait 10 seconds to be sure that is done scenario definition ..."

echo "--------------------------------------------------"
echo "Starting cdmw_test_entity_nav ..."
cdmw_test_entity_nav 


echo "--------------------------------------------------"
echo "kill supervision, FT Manager, Daemon ..."
