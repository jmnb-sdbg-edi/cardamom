      ...
      m_exit_required(false)
{
    ...
    {       
        std::auto_ptr <Cdmw::Event::EventChannel> p_eventchannel (
                                new Cdmw::Event::EventChannel (COMPLETE_CHANNEL_NAME,
                                                                nc.in()));
                                             
        try 
        {
            m_pushConsumer = p_eventchannel->get_pushConsumer(CosEventComm::PushSupplier::_nil());
        } 
        catch (...) 
        {
            std::cerr << "impossible to get a push consumer on " << COMPLETE_CHANNEL_NAME << std::endl;
        }
    }
}
