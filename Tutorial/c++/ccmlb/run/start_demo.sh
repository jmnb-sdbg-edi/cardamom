...
SITE_CONFIG=../../site.mk
SCENARIO_FILE=../data/ScenarioCcmLB.xml
...
if [ "$HOSTNAME1" = "" ]
then
    $echo "HOSTNAME1 is not set as environnement variable"
    exit 1
fi
...
if [ "$HOSTNAME2" = "" ]
then
    $echo "HOSTNAME2 is not set as environnement variable"
    exit 1
fi
...
if [ "$HOSTNAME1" = "" ]
then
    $echo "HOSTNAME3 is not set as environnement variable"
    exit 1
fi
...
rm -fr workDir
rsh $HOSTNAME2 rm -fr $CDMW_TUTORIAL_HOME/ccmlb/run/workDir
rsh $HOSTNAME3 rm -fr $CDMW_TUTORIAL_HOME/ccmlb/run/workDir
...
rm -fr $FTP_DIR/cdmw_qualif_ccm_lb
rsh $HOSTNAME2 rm -fr $FTP_DIR/cdmw_qualif_ccm_lb
rsh $HOSTNAME3 rm -fr $FTP_DIR/cdmw_qualif_ccm_lb
...
then
    $echo "$DAEMON_CONF_FILE file is missing. Run 'make data' in parent directory restart $0 "LD_LIBRARY_PATH
    exit 1
fi
...
DEMO_ALREADY_INSTALLED=`rsh $HOSTNAME1 ls $DEMO_DIR/$HOST_TYPE/ 2>/dev/null`
if [ "$DEMO_ALREADY_INSTALLED" = "" ]
then
rsh $HOSTNAME1 mkdir -p $CDMW_TUTORIAL_HOME
   $echo "  => Copying demo lb"
   rcp -r $CDMW_TUTORIAL_HOME/ccmlb $HOSTNAME1:$CDMW_TUTORIAL_HOME
else
...
# We determine the target
HOST_TYPE="`$CDMW_HOME/bin/config.guess`"
DEMO_ALREADY_INSTALLED=`rsh $HOSTNAME2 ls $DEMO_DIR/$HOST_TYPE/ 2>/dev/null`
if [ "$DEMO_ALREADY_INSTALLED" = "" ]
then
rsh $HOSTNAME2 mkdir -p $CDMW_TUTORIAL_HOME
   $echo "  => Copying ccmlb demo"
   rcp -r $CDMW_TUTORIAL_HOME/ccmlb $HOSTNAME2:$CDMW_TUTORIAL_HOME
else
...
# We determine the target
HOST_TYPE="`$CDMW_HOME/bin/config.guess`"
DEMO_ALREADY_INSTALLED=`rsh $HOSTNAME3 ls $DEMO_DIR/$HOST_TYPE/ 2>/dev/null`
if [ "$DEMO_ALREADY_INSTALLED" = "" ]
then
rsh $HOSTNAME3 mkdir -p $CDMW_TUTORIAL_HOME
   $echo "  => Copying ccmlb demo"
   rcp -r $CDMW_TUTORIAL_HOME/ccmlb $HOSTNAME3:$CDMW_TUTORIAL_HOME
else
...
rsh $HOSTNAME1 "(LANG=$LANG; export LANG; TAO_DISABLE_CODESET_ENCODING=$TAO_DISABLE_CODESET_ENCODING; export TAO_DISABLE_CODESET_ENCODING; LD_LIBRARY_PATH=$LD_LIBRARY_PATH; export LD_LIBRARY_PATH; PATH=$PATH; export PATH; cd $DEMO_DIR/run; export TAO_ROOT=$TAO_ROOT; $TERM_COMMAND1 -e $DAEMON_COMMAND1)" &
...
rsh $HOSTNAME2 "(LANG=$LANG; export LANG; TAO_DISABLE_CODESET_ENCODING=$TAO_DISABLE_CODESET_ENCODING; export TAO_DISABLE_CODESET_ENCODING; LD_LIBRARY_PATH=$LD_LIBRARY_PATH; export LD_LIBRARY_PATH; PATH=$PATH; export PATH; export TAO_ROOT=$TAO_ROOT; cd $DEMO_DIR/run; $TERM_COMMAND2 -e $DAEMON_COMMAND2)" &
...
rsh $HOSTNAME3 "(LANG=$LANG; export LANG; TAO_DISABLE_CODESET_ENCODING=$TAO_DISABLE_CODESET_ENCODING; export TAO_DISABLE_CODESET_ENCODING; LD_LIBRARY_PATH=$LD_LIBRARY_PATH; export LD_LIBRARY_PATH; PATH=$PATH; export PATH; cd $DEMO_DIR/run; export TAO_ROOT=$TAO_ROOT; $TERM_COMMAND3 -e $DAEMON_COMMAND3)" &
#rsh $HOSTNAME3 "(cd $DEMO_DIR/run; $TERM_COMMAND3 -e $DAEMON_COMMAND3)" &

sleep $INIT_TIMEOUT

# 1b) Start FT Manager
$echo Starting the FT Manager...
$TERM -sb -sl 5000 -geometry 95x21+0+312 -bg rgb:f0/e6/8c -title ${HOSTNAME}_ft_manager -e $CDMW_HOME/bin/cdmw_ft_manager --CdmwXMLFile=$CDMW_HOME/share/CdmwFaultToleranceManager_conf.xml --groupConf=../data/CdmwFTSystemMngtGroupCreator_conf.xml & 
FT_MANAGER_PID=$!
...
SUPERVISION_PID=$!
# instead you can use the line below
# this allow monitoring of supervision by daemon (Supervision FT case)
#$CDMW_HOME/bin/cdmw_platform_supervision_starter --CdmwXMLFile=$CDMW_TUTORIAL_HOME/ccmlb/data/CdmwPlatformMngtSystemStart.xml --validate
sleep $INIT_TIMEOUT
...
LB_GROUP_MANAGER_OBSERVER_COMMAND="$CDMW_HOME/bin/start_lb_group_manager_observer --system-corbaloc=corbaloc::$HOSTNAME:21871/CdmwPlatformMngtSupervision --CdmwLBXMLFile=$DEMO_DIR/data/CdmwLBGroupManager_ccmlb_conf.xml" 
rsh $HOSTNAME2 "(LANG=$LANG; export LANG; TAO_DISABLE_CODESET_ENCODING=$TAO_DISABLE_CODESET_ENCODING; export TAO_DISABLE_CODESET_ENCODING; LD_LIBRARY_PATH=$LD_LIBRARY_PATH; export LD_LIBRARY_PATH; PATH=$PATH; export PATH; $TERM_LB_COMMAND -e $LB_GROUP_MANAGER_OBSERVER_COMMAND) " &
sleep $INIT_TIMEOUT


# 3) Define system
$echo Defining the System '$SCENARIO_FILE'
$CDMW_HOME/bin/cdmw_platform_admin --system-corbaloc=corbaloc::$HOSTNAME:21871/CdmwPlatformMngtSupervision --sys-define $SCENARIO_FILE

# 4) Start_system
$CDMW_HOME/bin/cdmw_platform_admin --system-corbaloc=corbaloc::$HOSTNAME:21871/CdmwPlatformMngtSupervision --sys-start
if [ $? -eq "0" ];
then
...
$CDMW_HOME/bin/cdmw_platform_admin --system-corbaloc=corbaloc::$HOSTNAME:21871/CdmwPlatformMngtSupervision --sys-snapshot

# 6) stop_system
$CDMW_HOME/bin/cdmw_platform_admin --system-corbaloc=corbaloc::$HOSTNAME:21871/CdmwPlatformMngtSupervision --sys-stop
