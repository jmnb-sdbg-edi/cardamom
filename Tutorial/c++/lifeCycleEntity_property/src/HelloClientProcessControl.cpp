...
HelloClientProcessControl::HelloClientProcessControl (CORBA::ORB_ptr orb)
    throw(CORBA::SystemException)
    : m_orb (CORBA::ORB::_duplicate(orb)),
      m_hello_client_thread (NULL),
      m_applicationName (""),
      m_processName ("")
{
}

HelloClientProcessControl::~HelloClientProcessControl()
    throw()
{
    // ==================================================
...
void HelloClientProcessControl::on_initialise (
    ...
    throw(CORBA::SystemException)
{
    ...
    // ==================================================
    m_hello_client_thread =
        new Cdmw::Tutorial::HelloClientThread (m_orb.in(),
                                                m_applicationName,
                                                m_processName);
          ...
          CORBA::SystemException)
{
    ...
    if (m_hello_client_thread != NULL) 
    {
	    m_hello_client_thread->start();
	}           
}


// process to stop called by platformmngt
void HelloClientProcessControl::on_stop()
    throw(CORBA::SystemException)
{
    // =================================================================
    // example of using the PlatformInterface for notifying a message
...
// stop process
void HelloClientProcessControl::stop_thread()
    throw()
{
    try
    {
        if (m_hello_client_thread != NULL) 
        {
            ...
            Cdmw::OsSupport::ThreadStatus status = m_hello_client_thread -> get_status();
        
            if ((status == Cdmw::OsSupport::THREAD_RUN_REQUEST) ||
                (status == Cdmw::OsSupport::THREAD_RUNNING)) 
            {
                m_hello_client_thread->stop();
                m_hello_client_thread->join();
            }
        
	        delete m_hello_client_thread;
	        m_hello_client_thread = NULL;
        }
    }
    catch (...)
