...
#include <FaultTolerance/ftrequestlogging/CdmwFTRequestInfo.stub.hpp>
...
typedef Cdmw::CommonSvcs::DataStore::DataStoreObserver<std::string, CdmwFT::ReplyData> DsObserver;

class MyObserver : public DsObserver
{
 public:
    void dump_data(const char* hdr, 
                   const DsObserver::_Oid_in_type oid, 
                   const DsObserver::_Data_in_type data)
    {
        std::ostringstream ostr;
        ostr << "[ReplyLogging] " << hdr << " \n"
             << "\t oid = " << oid <<"\n"
             << "\t data.expiration_time= " << data.expiration_time << "\n"
             << "\t data.data= ";
        ACE_HEX_DUMP ((LM_DEBUG,
                       (const char *) data.data.get_buffer(),
                       const_cast<DsObserver::_Data_in_type>(data).data.length(),
                       ostr.str().c_str()));
    }

    virtual 
    void 
    on_insert(const DsObserver::_Oid_in_type oid, const DsObserver::_Data_in_type data)
        throw ()
    {
        dump_data("Inserting",oid,data);
    }

    virtual 
    void on_update(const DsObserver::_Oid_in_type oid, const DsObserver::_Data_in_type data)
        throw ()
    {
        dump_data("Updating",oid,data);
    }

    virtual 
    void on_remove(const DsObserver::_Oid_in_type oid)
        throw ()
    {
        std::cout << "[ReplyLogging] Deleting oid = " << oid << std::endl;
    }
};
...
{
    using namespace Cdmw::FT::RequestLogging;
    int ret_code = 0;
    ...
    if ( OS::get_option_value( argc, argv, "--client") == "yes" ) 
    {
        try
        {
            ...
            typedef Cdmw::CommonSvcs::DataStore::StdMapDataStoreFactory<std::string, CdmwFT::ReplyData> DsFactory;

            DsFactory::Create_data_store(DS_ID);

            Cdmw::CommonSvcs::DataStore::DataStoreBase* ds
                = Cdmw::CommonSvcs::DataStore::DataStoreRegistry
                ::Get_data_store(DS_ID);

            typedef Cdmw::CommonSvcs::DataStore::DataStoreInterface<std::string, CdmwFT::ReplyData> DsInterface;
            DsInterface * myds= dynamic_cast<DsInterface*>(ds);
            myds->register_observer(new MyObserver());

            Cdmw::Logging::LogManager::Init(argc, argv);

            ReplyRecording::ReplyStore* store =
                dynamic_cast<ReplyRecording::ReplyStore*>(ds);



            ReplyRecording* recorder = new ReplyRecording(*store);

            Cdmw::FT::FTServiceInit::Set_reply_recorder(recorder);
...
//             strategyList.add_multicast();
    ...
    else 
    {
        try
        {
            ...
            strategyList.add_multicast();
            // Initialises the ORB
//             char *args[] = { "cdmw_test_ft_logging", "-ORBDebugLevel", "10" };
//             int count = 3;
...
//             CORBA::ORB_var orb = Cdmw::OrbSupport::OrbSupport::ORB_init(count, args, strategyList, "main_orb");
            ...
            // Do test
            Cdmw::FT::TestRequestLogging myTest(orb.in(), rootPOA.in(),
                                                "TestRequestDataStore", DS_ID);
            myTest.start();
