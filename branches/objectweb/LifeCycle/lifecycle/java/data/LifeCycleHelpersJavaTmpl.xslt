...
-->
<xsl:template name="findAllJavaFactoryOperations">
   ...
   <xsl:for-each select="$containedOps">
      ...
      <xsl:choose>
         <xsl:when test="$_template = 'entity_factory_java.content.1'">
            <xsl:call-template name="entity_factory_java.content.1">
         ...
         <xsl:when test="$_template = 'unmanaged_entity_factory_java.content.1'">
            <xsl:call-template name="unmanaged_entity_factory_java.content.1">
               <xsl:with-param name="_node" select="."/>
               <xsl:with-param name="_returnType" select="$returnType"/>
               <xsl:with-param name="_operationName" select="$javaOperationName"/>
            </xsl:call-template>
         </xsl:when>
         <xsl:when test="$_template = 'service_factory_java.content.1'">
            <xsl:call-template name="service_factory_java.content.1">
               <xsl:with-param name="_node" select="."/>
               <xsl:with-param name="_returnType" select="$returnType"/>
               <xsl:with-param name="_operationName" select="$javaOperationName"/>
            </xsl:call-template>
         </xsl:when>
         <xsl:when test="$_template = 'session_factory_java.content.1'">
            <xsl:call-template name="session_factory_java.content.1">
               <xsl:with-param name="_node" select="."/>
               <xsl:with-param name="_returnType" select="$returnType"/>
               <xsl:with-param name="_operationName" select="$javaOperationName"/>
            </xsl:call-template>
         </xsl:when>
         <xsl:when test="$_template = 'servant_provider_java.content.1'">
            <xsl:call-template name="servant_provider_java.content.1">
               <xsl:with-param name="_node" select="."/>
               <xsl:with-param name="_operationName" select="$javaOperationName"/>
            </xsl:call-template>
         </xsl:when>
