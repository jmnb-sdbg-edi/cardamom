...
<!--
   ...
   @param none
-->
<xsl:template name="service_factory_cpp">
   <!--
      ...
      <xsl:if test="$_index &lt;= count($nodes)">
         ...
         <xsl:document method="text" href="{$filename}">
            ...
            <!--
               Insert the content.
            -->
            <xsl:call-template name="service_factory_cpp.content">
         ...
         -->
         <xsl:call-template name="service_factory_cpp">
            <xsl:with-param name="_index" select="$_index + 1"/>
         </xsl:call-template>
      </xsl:if>
   </xsl:if>
</xsl:template> <!-- end of template service_factory_cpp -->
...
-->
<xsl:template name="service_factory_cpp.content">
   ...
   try
   {
      ...
      <![CDATA[
         ...
         PortableServer::ServantLocator_var the_locator
            = new TheServantLocator(m_servant_provider);
         m_objects_poa -> set_servant_manager(the_locator.in());
      ...
      throw()
   {
      ...
      // m_objects_poa and m_servant_locator references will be released by their destructors;
      // and the associated POA and ServantLocator implementations will be destroyed following
   ...
   <xsl:call-template name="findAllFactoryOperations">
      ...
      <xsl:with-param name="_template" select="'service_factory_cpp.content.1'"/>
...
</xsl:template> <!-- end of template service_factory_cpp.content -->



<!--
-->
<xsl:template name="service_factory_cpp.content.1">
