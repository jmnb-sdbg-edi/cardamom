...
#include "testrepositoryext/TestNamingContext.hpp"
...
public:
	...
	*/
        ...
        throw(CdmwPlatformMngt::Process::BadOrder, CORBA::SystemException)
    {
		...
		std::string parameters = "--CdmwLocalisationService=4997 --id=GlobalRepository --conf=CdmwDefaultNamingAndRepository.xml --CdmwXMLFile=CdmwNamingAndRepositoryDatastoreConfig.xml";

                std::cout << "Launch the repository with options: "  
			  << parameters
			  << " with cwd==" << CWD
			  << std::endl;
        ...
        {
            std::cout << "Getting repository ext from IOR file" << std::endl;

            std::string repositoryIOR;
            std::ifstream iorFile("RepositoryExt.IOR");
        ...
        m_repository = CdmwNamingAndRepository::RepositoryExt::_narrow(repository_temp.in());
	...
	*/
              ...
              CORBA::SystemException)
    {
        ...
        TestNamingContext testrepositoryext("Naming Context", m_orb.in(), m_repository.in());

        testrepositoryext.start();
