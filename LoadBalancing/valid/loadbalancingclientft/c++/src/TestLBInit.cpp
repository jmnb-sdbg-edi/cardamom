...
void TestLBInit::do_tests()
{
    ...
    std::string dummy;
    std::cout<<"Kill the GroupManager Primary (FTGroupManager1 process)"<<std::endl;
  ...
  // add new member
    ...
    // use group manager method to recover the next object group reference
    // (REQ 259 - 261)
    try
    {
	TEST_INFO("use group manager method to check the good object group version");
    ...
    TEST_INFO("Test the recovering of replica crashed...");
    TEST_INFO("re-start process FTGroupManager1");
    std::cout<<"Press any key and Enter"<<std::endl;
    std::cin >> dummy;
    ...
    TEST_INFO("Calling hello on HelloInterface...the FTManager should update the IOGR with the re-started replica");
    try
    {   
      CORBA::Long id = 0;
      for(int i = 0; i < 10; i++)
    ...
    TEST_INFO("kill the FTGroupManager2 and FTGroupMananger3");
    std::cout<<"Press any key and Enter"<<std::endl;
    std::cin >> dummy;
    ...
    version_HelloInterface = m_group_manager->get_object_group_version_from_ref(hello_group_HelloInterface.in());    
    ...
    std::cout<<"OLD object_group_version :"<<helloInterface_version<<std::endl;
  ...
  // remove the menber previously deleted
    ...
    TEST_INFO("Remove a member P11 already killed from the helloInterface (Member Not Found Exception should be raised)");
