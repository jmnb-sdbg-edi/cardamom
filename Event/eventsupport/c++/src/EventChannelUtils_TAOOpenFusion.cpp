...
namespace
{
    ...
    const char* TAO_CEC_PROPERTY = "PROP";
};
...
/********************************************************************/
// Get TAO CEC factory properties 
// This will read event channel profile if any and return number of
// properties and list of properties.
//
std::string
EventChannelUtils::get_factory_properties(const char* channel_name)
          ...
          CORBA::SystemException)
{
    ...
    manager = CdmwEventAdmin::EventChannelProfileManager::_narrow(obj.in());
    ...
    if (channelProfile)
    {
        ...
        for (i_property=0; i_property<nb_properties; i_property++)
        {
            ...
            if (propertyName == TAO_CEC_PROPERTY)
            {
                CECproperties = (*channelProfile)[i_property].value;
