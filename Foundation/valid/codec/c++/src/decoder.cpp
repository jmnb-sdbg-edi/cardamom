...
namespace 
{
    ...
    public:
            ...
            throw(CORBA::SystemException)
        {
            ...
            Cdmw::TestCodec::DecoderInit::init(m_orb.in());            
        }
...
{    
    ...
    std::cout << "decoder run with CdmwLocalisationService=" << portNumber << std::endl;

    std::cout << "Start of decoder" << std::endl;    

    CORBA::ORB_var orb;                        // orb reference

    std::string strArgv = "valid_decoder";
    int myArgc(1);
    ...
    std::cout << "Initialize the ORB" << std::endl;
        ...
        // decoder ORB_init must be done with CdmwLocalisationService so we use argv
        // instead of myArgv : CALLBACK_IOR_OPTION is needed only for platform setup
