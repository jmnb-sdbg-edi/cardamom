...
namespace Cdmw
{
        ...
        throw (CORBA::SystemException, PortableInterceptor::ForwardRequest)
    {
        ...
        if (m_event_to_intercept == kIntercept_RECEIVE_REQUEST) 
        {
            ...
            {
                if ((m_host_name == "APPL1") &&
                    (m_expected_intercept_count == m_actual_intercept_count))
                {
                  std::cout << m_host_name << "  -> count = " << m_actual_intercept_count << " *>>>> waiting timeout <<<<" << std::endl;
                  Cdmw::OsSupport::OS::sleep(13000);
                }
                else 
                {
