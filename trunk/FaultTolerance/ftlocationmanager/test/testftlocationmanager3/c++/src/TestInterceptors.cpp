...
//                             , int expected_event_count, const char* method
        ...
        throw (CORBA::SystemException, PortableInterceptor::ForwardRequest)
    {
        ...
        std::cout << "[**** " << m_process_name << "::TestClientInterceptor] send_request(" << actual_method << "):" << std::endl;
    ...
    {
        std::string actual_method = ri->operation();
        std::cout << "[**** " << m_process_name << "::TestClientInterceptor] receive_reply(" << actual_method << "):" << std::endl;
