...
#include "testftprimary/TestPrimary.hpp"
...
;
        ...
        //
        // This validation test checks that the first member added to
        // an object group becomes the primary member.
        //
        ...
        // Check that PROC1 is not yet primary

        TEST_INFO("Checking that helloPROC11 is not primary")
        try
        {
            TEST_CHECK(!helloPROC11->is_primary(helloPROC11.in()));
        }
        catch (...)
        {
            TEST_FAILED();
        ...
        // Add group member PROC1. PROC1 should become the primary process.
        TEST_INFO("[---- TestPrimary::do_tests] add the member "<<m_host1.c_str()<<"/APPL1/P11 in group 1");
        ...
        // Check that PROC1 is primary

        TEST_INFO("Checking that helloPROC11 is now the primary")
        try
        {
            TEST_CHECK(helloPROC11->is_primary(helloPROC11.in()));
        }
