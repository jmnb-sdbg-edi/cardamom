...
<!--
   ...
   @param none
-->
<xsl:template name="ft_session_home_hpp">
   <xsl:call-template name="session_home_hpp">
...
<!--
   ...
   @param none
-->
<xsl:template name="session_home_hpp">
      ...
      <xsl:if test="$_index &lt;= count($lifecycleFrameworkNodes/home-impl)">
         ...
         <xsl:document method="text" href="{$filename}">
            ...
            <!--
               Insert the content.
            -->
            <xsl:call-template name="session_home_hpp.content">
...
</xsl:template> <!-- end of template session_home_hpp -->
...
-->
<xsl:template name="session_home_hpp.content">
...
#include <CCMContainer/ccmcif/FTCCMHome_impl.hpp>
                                                       ...
                                                       virtual public HomeConfiguration_impl
   {
      ...
      public:
         // Destructor
         ~<xsl:value-of select="$homeImplClassname"/>() throw();
         ...
         <xsl:call-template name="findAllInterfaceReadWriteAttributes">
            ...
            <xsl:with-param name="_template" select="'session_home_hpp.content.2'"/>
         </xsl:call-template>
         ...
         <xsl:call-template name="findAllHomeFactoryOperations">
            ...
            <xsl:with-param name="_template" select="'session_home_hpp.content.1'"/>
         </xsl:call-template>
         ...
         // CDMW Keyless home operation
         Components::CCMObject_ptr
         create_component_with_config_values(const <![CDATA[Components::ConfigValues&]]> config)
            throw(Components::CreateFailure,
                  CORBA::SystemException);
   ...
   namespace Container
   {
      ...
      HomeAllocator<![CDATA[<]]><xsl:value-of select="concat('CIF::Cdmw', $cppHomeScope, $cppSep, $homeImplClassname)"/><![CDATA[>::allocate]]>
         ...
         throw(CORBA::SystemException,
               std::bad_alloc)
      {
         return new <xsl:value-of select="concat('CIF::Cdmw', $cppHomeScope, $cppSep, $homeImplClassname)"/>(home_executor, deactivator, config);
      }
...
</xsl:template> <!-- end of template session_home_hpp.content -->



<!--
-->
<xsl:template name="session_home_hpp.content.1">
   ...
   <xsl:value-of select="concat($lf, 'virtual ', $_returnType, $lf, $_operationName, $lf)"/>
      ...
      throw (Components::CreateFailure,
             CORBA::SystemException);
</xsl:template> <!-- end of template session_home_hpp.content.1 -->


<xsl:template name="session_home_hpp.content.2">
   <xsl:param name="_node"/>
...
</xsl:template> <!-- end of template session_home_hpp.content.2 -->
