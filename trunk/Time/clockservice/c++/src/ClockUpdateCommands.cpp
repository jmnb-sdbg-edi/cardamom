...
{
    m_controlled_clock.set_(data.X0, data.Y0, data.frozen_time);
}

DEFINE_CTRL_CLOCK_COMMAND(SetRateCommand)
{
    m_controlled_clock.set_rate_(data.rate);
}

DEFINE_CTRL_CLOCK_COMMAND(PauseCommand)
{
    m_controlled_clock.pause_(data.frozen_time);
}

DEFINE_CTRL_CLOCK_COMMAND(ResumeCommand)
{
    m_controlled_clock.resume_();
}

DEFINE_CTRL_CLOCK_COMMAND(TerminateCommand)
{
    m_controlled_clock.terminate_();
}
