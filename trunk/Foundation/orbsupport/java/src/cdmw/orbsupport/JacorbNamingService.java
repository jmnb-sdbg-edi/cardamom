...
import cdmw.ossupport.ThreadedProcess;
 ...
 public class JacorbNamingService {
    ...
    {
       try{
 	    String port = OS.getOptionValue(args, "-p");
	   
	    if (port == "no") 
			proc = OS.createProcess("Naming_Service -ORBEndpoint iiop://localhost:5000");
		else
	        proc = OS.createProcess("Naming_Service -ORBEndpoint iiop://localhost:" + port);
