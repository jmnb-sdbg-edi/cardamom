...
PACK_TMP_DIR=/tmp/CcmPerfPack_tmp
...
cp $PARENT_DIR/data/CcmPerf_1host.cad $PACK_TMP_DIR/meta-inf
cp $PARENT_DIR/data/*.cpf $PACK_TMP_DIR
cp $PARENT_DIR/data/*.ccd $PACK_TMP_DIR
cp $PARENT_DIR/data/*.csd $PACK_TMP_DIR

# copy idl file
echo idl...
cp $PARENT_DIR/idl/TestCcmMarshalling.idl $PACK_TMP_DIR

# make links for libraries
ln -s $PARENT_DIR/$HOST_TYPE/libServerCcmPerf.so $PACK_TMP_DIR/libServerCcmPerf.so
