...
#include "lifecycle/AnonymousResourceFactory.stub.hpp"
...
TestEntityStrategy::TestEntityStrategy(const std::string & args, 
                                       std::ostream &      os)
    throw()
    :Testable("Cdmw::LifeCycle::TestEntityStrategy",os),
     m_args(args)
{
    // does nothing
}
...
TestEntityStrategy::~TestEntityStrategy()
    throw()
{
    // does nothing
}
...
TestEntityStrategy::TestEntityStrategy(const TestEntityStrategy& rhs)
    throw()
...
TestEntityStrategy&
TestEntityStrategy::operator=(const TestEntityStrategy& rhs)
    throw()
{
    if (this != &rhs) {
        m_args    = rhs.m_args;
...
void TestEntityStrategy::do_tests()		
{
    ...
    try {
        ...
        std::string dev_key              = get_ENTITY_DEVICE_KEY();
        std::string dev_label            = get_ENTITY_DEVICE_LABEL();
        std::string appl_1_key           = get_ENTITY_APPLICATION_I_KEY();
        std::string appl_1_name          = get_PROGRAM_EXECUTABLE_1();
        std::string appl_2_key           = get_ENTITY_APPLICATION_II_KEY();
