...
#include "lifecycle/UserResourceServantProvider.hpp"
    ...
    throw(CF::Device::InvalidLocation)
{
    try {
        ...
        m_aom[key] = info;
        ECHO_DEF << "DEVICE <" << key << "> created with LABEL='" 
             << label << "'" << std::endl;
        return oid._retn();
    } catch (const CF::Device::InvalidLocation & ) {
        throw; // rethrow
    ...
    throw(CF::CreateApplicationError)
{
    try {
        ...
        m_aom[key] = info;
        ECHO_DEF << "APPLICATION <" << key << "> created with NAME='" 
             << name << "'" << std::endl;
        return oid._retn();
          ...
          CORBA::SystemException)
{
    try  {
        ...
        if (serv_pos  == m_aom.end()) {
            ...
            ECHO_DEF << "Activating " << key << " ... " << std::endl;
