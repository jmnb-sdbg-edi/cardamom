...
void TraceClientThread::run () throw()
{
    ...
    {        
        ...
        std::string msg;
        long count_for_trace = -1;
        long count_for_perf = 0;
        bool trace_domain2 = true;
        ...
        while (!m_exit_required) 
        {
            ...
            if (m_trace_count==-1 || count_for_trace < m_trace_count)
            {
                ...
                if (count_for_trace % 5 == 0) {
                    trace_domain2 = !trace_domain2;
                }

                if (m_multi_domain_level)
                {
                    if (trace_domain2) {
                        CDMW_TRACE("DOMAIN-2", 4,
                                   msg.c_str() << (count_for_trace / 10));
                    } else {
                        CDMW_TRACE("DOMAIN-1", 1,
                                   msg.c_str() << (count_for_trace / 10));
                    }
                }
