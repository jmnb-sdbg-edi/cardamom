...
<!--
   ...
   @param none
-->
<xsl:template name="service_factory_hpp">
   <!--
      ...
      <xsl:if test="$_index &lt;= count($nodes)">
         ...
         <xsl:document method="text" href="{$filename}">
            ...
            <!--
               Insert the content.
            -->
            <xsl:call-template name="service_factory_hpp.content">
         ...
         -->
         <xsl:call-template name="service_factory_hpp">
            <xsl:with-param name="_index" select="$_index + 1"/>
         </xsl:call-template>
      </xsl:if>
   </xsl:if>
</xsl:template> <!-- end of template service_factory_hpp -->
...
<!--
   ...
   @param _scopedInterfaceName              The scoped anonymous interface name.
   @param _interfaceFilename                The anonymous interface filename.
...
-->
<xsl:template name="service_factory_hpp.content">
      ...
      public virtual Cdmw::OrbSupport::RefCountServantBase
   {
      public:
         ...
         <xsl:call-template name="findAllFactoryOperations">
            ...
            <xsl:with-param name="_template" select="'service_factory_hpp.content.1'"/>
      ...
      private:
         ...
         Cdmw::LifeCycle::NoRetainObjectDeactivator m_object_deactivator;

         <xsl:value-of select="$cppScopedServantProviderClassname"/>* m_servant_provider;
   }; // End class <xsl:value-of select="$_factoryImplClassname"/>
...
</xsl:template> <!-- end of template service_factory_hpp.content -->



<!--
-->
<xsl:template name="service_factory_hpp.content.1">
