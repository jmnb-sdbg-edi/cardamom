...
#include <iostream>
#include <limits>
#include "testvalidpersistenceuserdefinedattributes/TestUserDefinedAttributes.hpp"
 ...
 try
	...
	CdmwPlatformMngt::ElementNames_var host_names;
        ...
        catch(...)
        {
                ...
                TEST_INFO("ERROR RECOVERING THE HOST NAMES");
        }

	TEST_INFO("RECOVERED HOSTNAMES ");
	TEST_INFO((*host_names)[0]);		
        TEST_INFO((*host_names)[1]);


	myhost = mysystem -> get_host((*host_names)[1]);

	TEST_INFO("RESOLVING PROCESS HelloProcess ");
	myproc = myhost->get_process("HelloProcess1", "HELLO_WORLD");
        ...
        TEST_INFO("RESOLVING ENTITY associated to SYSTEM");
	...
	try
	  {
	    ...
	    TEST_INFO("PERSISTENCE USERDEFINED ATTRIBUTE ON SYSTEM ");
	    ...
	    TEST_INFO("Any value recovered from PROCESS with value:  "<<l_proc_out);
            ...
            short l_system_entity_out;  
            (*return_any_val_system_entity) >>= l_system_entity_out;
