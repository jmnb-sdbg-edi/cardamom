...
static const char* p_outFlushAreaMsg = "trace Flush Area Manager is running out of flush areas";
...
#	    endif
		...
		try
		{
		  ...
		  PortableServer::ObjectId_var oid = tracePOA->activate_object(pTraceProducerServant);
		...
		std::list<CdmwTrace::TraceProducer::CollectorData>::iterator itCollectorList = 
				                            collectorList.begin();
		...
		for (; itCollectorList != collectorList.end() ; itCollectorList++)
		{
		  ...
		  try
		  {
		    ...
		    CdmwTrace::Collector_var collector = itCollectorList->the_collector;
	        // get collector object name from list
		    collector_obj_name = itCollectorList->the_collectorObjName.in();
	        // get collector mnemonic name from list
		    std::string collector_mnemonic = itCollectorList->the_collectorMnemoName.in();
	      ...
	      {
            std::cerr << "Trace Collector access error : " << collector_obj_name.c_str() << "\n";
            print_exception(ex);
	      }
        }
           ...
           InternalErrorException)
{
    ...
    return CdmwTrace::TraceProducer::_duplicate(m_pSingleton->m_traceProducer.in());
}
	...
	try
	{
      ...
      while (! m_thread_must_terminate) 
	  {
        ...
        if (status == Condition::TIMEDOUT)
        {
            ...
            {
              std::cerr << "Exception raised in trace library" << std::endl;
            }
...
FlushAreaMngr& FlushAreaMngr::instance()
    throw ()
{
...
bool FlushAreaMngr::add_message (const Message::MessageHeader& header, const std::string& body)
     throw (InternalErrorException)
{
    bool ret = false;
    ...
    try
	{
      ...
      do 
	  {
       	...
       	if (m_pCurrentFlushArea == NULL)
        {
            ...
            CDMW_TRACE_DEBUG("No more flush area available at the moment" << std::endl);
            break;
    	}

        FlushArea::AreaStatus status = m_pCurrentFlushArea->add_message (header, body);
        ...
        switch (status)
        {
            ...
            {
                CDMW_TRACE_DEBUG("Warning : Cdmw::Trace::FlushAreaMngr :");
                CDMW_TRACE_DEBUG("Inappropriate area size. Cannot manage message with body length of " << body.length() << " characters." << std::endl);
            }
            break;
	    } 
...
//
//
void FlushAreaMngr::deliver_flushArea ()
    throw (OutOfMemoryException,
           InternalErrorException)
{
    // BE CAREFUL: m_currentFlushArea_mtx mutex must have been
...
//
//
void FlushAreaMngr::update_currentFlushArea()
    throw()
{
    try
	{
...
void FlushAreaMngr::put_flushedArea (FlushArea* pFlushedArea)
    throw (OutOfMemoryException)
{
...
//
void FlushAreaMngr::create_FlushAreas (size_t nbFlushArea, 
				                       size_t sizeFlushArea, 
    ...
    throw (OutOfMemoryException)
{
    try
    {
      ...
      m_pCurrentFlushArea = 
				new FlushArea (areaNbMessage, sizeFlushArea, sizeOutFlushAreaMsg, 0);

      for (unsigned int iArea = 1 ; iArea < nbFlushArea ; ++iArea)
      {
        m_vpFlushArea.push_back (
				   new FlushArea (areaNbMessage, sizeFlushArea, sizeOutFlushAreaMsg, iArea));  
      }
    }
    catch (const std::bad_alloc&)
    {
...
//
void FlushAreaMngr::delete_FlushAreas()
    throw()
{
    try
	{
...
//
void FlushAreaMngr::activate_flushing ()
    throw()
{
    m_flush_active = true;
...
//
void FlushAreaMngr::deactivate_flushing ()
    throw()
{
    m_flush_active = false;
...
//
void FlushAreaMngr::set_flushMode (FlushMode flushMode)
    throw()
{
    try
	{
...
//
void FlushAreaMngr::activate_level (const std::string& domain, long level) 
    throw (OutOfMemoryException)
...
//
void FlushAreaMngr::deactivate_level (const std::string& domain, long level)
    throw (OutOfMemoryException)
...
//
bool FlushAreaMngr::is_to_be_traced (const std::string& domain, long level)
    throw (OutOfMemoryException)
{
    ...
    {
      msg_to_trace =
			  m_pSingleton->m_pFilterMngr->is_activated (domain, level);
    }
