...
then
    echo "Usage :"
    echo "$0 validation_scenario [--exectime=<time_sec>]"
    exit
fi
...
while [ -n "$2" ]; do
      case "$2" in
         --exectime=*)
            EXEC_TIMEOUT=`echo $2 | sed 's/[-_a-zA-Z0-9]*=//'`
            echo "EXEC Timeout set to " $EXEC_TIMEOUT
            ;;
         --keystop)
            KEYSTOP=YES
            echo "The scenario will be stop by a keypressed"
            ;;
...
then
    if [ "$KEYSTOP" = "YES" ];
    then
        echo "Press a key to stop scenario when the test is finished"
        read FOO
    else
...
NB_ERR1=`grep -c FAILED $GLOBAL_TESTS_FILE`
NB_ERR2=`grep -c FAILURE $GLOBAL_TESTS_FILE`
