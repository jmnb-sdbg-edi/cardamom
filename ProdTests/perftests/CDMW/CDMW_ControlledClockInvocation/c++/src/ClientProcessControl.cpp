...
namespace ControlledInvocation
{
        ...
        throw(CORBA::SystemException)
    {
        ...
        //=====================================================================
        CosClockService::ClockCatalog::ClockEntry_var entry = 
			clock_catalog->get_entry("ControlledClock");
		m_controlledClock =
        ...
        if (CORBA::is_nil(m_controlledClock.in()))
            {
                std::cerr << "Client:>> Unable to resolve ControlledClock"
                          << std::endl;
                exit(1);
            }
    } // on_initialise()
...
}; // End namespace ControlledInvocation
